...
export default function ExitoScreen() {
  ...
  // Extraemos useRowLayout de useScale para saber si estamos en un contenedor ancho
  const { contentWidth, scale, useRowLayout } = useScale({ 
    maxContentWidthTablet: 600, 
    tabletWidthRatio: 0.8, 
    maxScale: 1.25 
  });
  ...
  // Optimizamos el renderizado de filas para soportar saltos de línea elegantes si el texto es largo
...
const createStyles = (scale) =>
  StyleSheet.create({
    ...
    scrollContainer: { flexGrow: 1, paddingVertical: 32 * scale, justifyContent: 'center' },
    ...
    // El diseño de fila se vuelve más seguro en dispositivos estrechos con flexWrap
    row: { 
      flexDirection: 'row', 
    ...
    homeButton: { 
      ...
      alignItems: 'center', 
      justifyContent: 'center',
      alignSelf: 'stretch' // Se expande adecuadamente respetando el contentWidth
    },
