...
export default function PhotoCaptureScreen() {
  ...
  const bothCaptured = !!proveedorPhoto && !!idPhoto;
      ...
      {photo ? (
        ...
        <>
          <View style={s.cameraIconCircle}>
            <Ionicons name="camera-outline" size={28 * scale} color="#1e3a68" />
          </View>
        ...
        <View style={[s.container, { width: contentWidth }]}>
          ...
          <View style={[s.bottomSection, (isLandscape || isTablet) && s.bottomSectionLandscape]}>
            <View style={[s.captureContainer, useRowLayout && s.captureContainerRow]}>
              ...
              {renderCaptureBox('id', idPhoto, 'Identificación oficial')}
            </View>

            <Text style={s.footerInstruction}>
              {bothCaptured
                ? 'Ambas fotografías fueron capturadas'
                : 'Ambas fotografías son requeridas para continuar'}
            </Text>

            <TouchableOpacity
              style={[s.continueButton, !bothCaptured && s.continueButtonDisabled]}
...
const createStyles = (scale) =>
  StyleSheet.create({
    ...
    captureBoxFilled: { borderStyle: 'solid', borderColor: '#00a884' },
    captureBoxRow: { flex: 1 },
    cameraIconCircle: { width: 56 * scale, height: 56 * scale, borderRadius: 28 * scale, backgroundColor: '#f0f4f8', alignItems: 'center', justifyContent: 'center', marginBottom: 10 * scale },
    captureLabel: { color: '#1e3a68', marginTop: 4, fontWeight: 'bold', fontSize: 14 * scale },
    captureHint: { color: '#9ca3af', fontSize: 12 * scale, marginTop: 4 },
    capturedImage: { width: '100%', height: '100%' },
    retakeBadge: { position: 'absolute', bottom: 10, right: 10, backgroundColor: 'rgba(26,53,91,0.85)', padding: 8, borderRadius: 20 },
    checkBadge: { position: 'absolute', top: 10, left: 10, backgroundColor: '#00a884', padding: 5, borderRadius: 12 },
