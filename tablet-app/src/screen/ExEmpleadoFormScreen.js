...
export default function ExEmpleadoFormScreen() {
  ...
  const [motivo] = useState('Finiquito'); // Fijo por defecto para la base de datos
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert('Error', error.message || 'No hay conexión con el servidor o falló el guardado.');
    } finally {
      setIsLoading(false);
    }
  };
...
const createStyles = (scale) =>
  StyleSheet.create({
    ...
    exEmpleadoCard: { backgroundColor: '#eff6ff', padding: 16 * scale, borderRadius: 12, marginBottom: 24 * scale, marginTop: 10 * scale, borderWidth: 1, borderColor: 'rgba(59, 130, 246, 0.25)' },
    exEmpleadoTitle: { color: '#1e40af', fontSize: 11 * scale, fontWeight: 'bold', letterSpacing: 1 },
